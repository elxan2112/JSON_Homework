const achivmentsMock = [
    ...
    let main = achivmentsMock.map(all =>{
        ...
        temp = Object.values(all.config).map(sub=>{
            ...
            result2.push(`id is ${all.id}, type is ${all.type}, name is ${sub.name}, isActive is ${sub.isActive}`)
            }
            if(sub.progress.maxPosition>=5){
                result3.push(`id is ${all.id}, type is ${all.type}, name is ${sub.name} with max pos >=5 - ${sub.progress.maxPosition}.`)
            }
        })
    })
